...
impl Mortgage {
    ...
    pub fn new(original_principal: i64, down_payment: i64, interest_rate: f64, loan_term_months: i32)
               -> Self {
        Self {
            original_principal,
            down_payment,
    ...
    fn minimum_monthly_payment(&self) -> i64 {
        self.original_principal * (1 + self.interest_rate) ^ (self.loan_term_months) /
            ((1 + self.interest_rate) ^ (self.loan_term_months) - 1)
    }

    // 4. Calculate the total amount paid after n months
    fn total_payout(&self, months: i32, monthly_payment: i64) -> i64 {
        (months * monthly_payment) as i64
    }
